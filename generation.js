...
const generation = {
    ...
    validField() {
        ...
        const {marked, unmarked} = this.highlightField
        //rows
        for (let i = 0; i < 9; i++) {
            if (this.rows[i][c].textContent === this.rows[r][c].textContent) {
                marked.bind(this)(i, c);
                marked.bind(this)(r, c);
            } else {
                unmarked.bind(this)(i, c);
                unmarked.bind(this)(r, c);
            }
        }
        //columns
        for (let i = 0; i < 9; i++) {
            if (this.rows[r][i].textContent === this.rows[r][c].textContent) {
                marked.bind(this)(r, i);
                marked.bind(this)(r, c);
            } else {
                unmarked.bind(this)(r, i);
                unmarked.bind(this)(r, c);
            }
        }
        //box
        ...
        for (let i = this.boxRow; i < this.boxRow + 3; i++) {
            for (let j = this.boxCol; j < this.boxCol + 3; j++) {
                if (this.rows[i][j].textContent === this.rows[r][c].textContent) {
                    marked.bind(this)(i, j);
                    marked.bind(this)(r, c);
                } else {
                    unmarked.bind(this)(i, j);
                    unmarked.bind(this)(r, c);
                }
            }
        }
